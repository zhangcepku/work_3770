...
TH1F *chi6(TFile *f1,int op,double cut, int cut2){
...
*/
    ...
    t1->SetBranchAddress("count_phi2",&count_phi2);
    t1->SetBranchAddress("count_phi1",&count_phi1);
    //-----------------------------------.rec only
    ...
    TH1F *h2 = new TH1F("Invariant_Mass_2","Invariant_Mass_2",20,0.10,0.16);
...
int m = 0;
        ...
        if (Chisq1<=50 && mass2>=0.1 && mass2 <=0.16){
            ...
            //if(the_decay<1.0){
            if (the_decay<cut && count_cone<=cut2){
            //if (the_decay<cut){
